...
prompt = PromptTemplate(template="""文章を元に質問を答えてください。
                        文章：
                        {documents}
...
async def on_chat_start():
    ...
    await cl.Message(content=f"'{file.name}'の読み込みが完了しました。質問を入力して下さい。").send()
...
async def on_message(input_message):
    ...
    result = chat([
        HumanMessage(content=prompt.format(documents=documents_string, query=input_message))
    ])
